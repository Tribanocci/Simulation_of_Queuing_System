...
//-----------MODEL constructor--------------------------
Model::Model(float T, int S_C, float S_R, int Ser_Num) { 
	T_SIM = T; Server_Num = Ser_Num; //Model Parameters Initialization
	Model_Source = new Source(S_R, S_C);  //Source Initialization
	int Ser_Cap; float Ser_Rate;
	for (int i{0}; i < Server_Num; i++){
		std::cout << "\nGive Server No. " << i+1 << " Capacity:\t";
		std::cin >> Ser_Cap;
		std::cout << "Give Server No. " << i+1 << " Process Rate:\t";
		std::cin >> Ser_Rate;
		Server_Vector.emplace_back(Ser_Rate, Ser_Cap); //Server Initialization
	}
...
Model_MMnK::Model_MMnK(float T, int S_C, float S_R, int Ser_Num) : Model(T, S_C, S_R, Ser_Num) {
	for (int i{0}; i < Server_Num; i++) {
