...
class Model {
	public:
		Model(float T, int S_C, float S_R, int Ser_Num, std::vector<int> Ser_Capacities, std::vector<float> Ser_Rates);
		~Model();
		//----------Model Outputs----------------------
...
//INDEX NOTATION, Index (0, Server_Num-1): is for proceess from servers.  Index Server_Num: is for arrival**.
		...
		Model_MMnK(float T, int S_C, float S_R, int Ser_Num, std::vector<int> Ser_Capacities, std::vector<float> Ser_Rates);
